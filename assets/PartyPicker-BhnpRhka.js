...
/* الجسم: الأصناف والإجماليات بجانب لوحة المعلومات مباشرة */
.body { display: flex; gap: 12px; align-items: stretch; }
.main { flex: 1; min-width: 0; }
.side { width: 218px; flex-shrink: 0; display: flex; flex-direction: column; gap: 8px; }
.side-box { background: #f8fafc; border: 1.5px solid #e2e8f0; border-radius: 12px; padding: 9px 12px; font-size: 12px; }
.side-box .k { color: #64748b; font-size: 10.5px; font-weight: 700; margin-bottom: 2px; }
.side-box .v { font-weight: 800; word-break: break-word; }
...
<tbody>${d||`<tr><td colspan="5" style="text-align:center;color:#94a3b8">لا توجد أصناف</td></tr>`}</tbody></table>
<div class="bottom">
...
<div class="side">
<div class="side-box"><div class="k">${n?`العميل`:`المورد`}</div><div class="v">${_(e.party_name||`عميل نقدي`)}</div></div>
<div class="side-box"><div class="k">التاريخ${e.due_date?` / الاستحقاق`:``}</div><div class="v">${o(e.date)}${e.due_date?` / ${o(e.due_date)}`:``}</div></div>
<div class="side-box"><div class="k">الفرع</div><div class="v">${_(e.branch_name||`—`)}</div></div>
<div class="side-box"><div class="k">الدفع</div><div class="v">${_(h[e.payment_method||``]||e.payment_method||`—`)}</div></div>
<div class="side-box"><div class="k">الحالة</div><div class="v"><span class="status-pill">${g[e.status||``]||e.status||`—`}</span></div></div>
${e.contentQrDataUrl?`<div class="qr-box" style="margin-top:2px"><img src="${e.contentQrDataUrl}" alt="QR" /><div class="cap">الأصناف والبيانات</div></div>`:``}
