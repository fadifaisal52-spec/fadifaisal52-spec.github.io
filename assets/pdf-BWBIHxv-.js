import{r as e}from"./invoicePrint-CvvfGU7z.js";function t(e){let{title:t,columns:n,rows:r,dateRange:i,vatNo:a}=e,o=i?`<p>الفترة: ${i.from} — ${i.to}</p>`:``,s=n.map(e=>`<th>${e.label}</th>`).join(``),c=r.map(e=>`<tr>${n.map(t=>`<td>${e[t.key]??``}</td>`).join(``)}</tr>`).join(``);return`<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><title>${t}</title>
<style>
