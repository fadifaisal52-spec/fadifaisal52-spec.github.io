...
/* الجسم: عمود بيانات رئيسي واحد بجانب الأصناف والإجماليات */
.body { display: flex; gap: 12px; align-items: stretch; }
.main { flex: 1; min-width: 0; }
.info-col { width: 215px; flex-shrink: 0; display: flex; flex-direction: column; border: 1.5px solid #0f172a; border-radius: 12px; overflow: hidden; background: #fff; }
.info-col .cell { padding: 9px 12px; border-bottom: 1px solid #e2e8f0; }
.info-col .cell:last-child { border-bottom: none; }
.info-col .k { color: #64748b; font-size: 10.5px; font-weight: 700; margin-bottom: 2px; }
.info-col .v { font-weight: 800; font-size: 13px; word-break: break-word; }
.info-col .num { font-size: 15px; }
.info-col .qr-cell { padding: 10px; text-align: center; background: #f8fafc; }
...
<div class="body">
<div class="info-col">
<div class="cell"><div class="k">رقم الفاتورة</div><div class="v num">${_(e.number||`—`)}</div></div>
<div class="cell"><div class="k">${n?`العميل`:`المورد`}</div><div class="v">${_(e.party_name||`عميل نقدي`)}</div></div>
<div class="cell"><div class="k">الفرع</div><div class="v">${_(e.branch_name||`—`)}</div></div>
<div class="cell"><div class="k">الحالة</div><div class="v"><span class="status-pill">${g[e.status||``]||e.status||`—`}</span></div></div>
<div class="cell"><div class="k">التاريخ${e.due_date?` / الاستحقاق`:``}</div><div class="v">${o(e.date)}${e.due_date?` / ${o(e.due_date)}`:``}</div></div>
<div class="cell"><div class="k">الدفع</div><div class="v">${_(h[e.payment_method||``]||e.payment_method||`—`)}</div></div>
${e.contentQrDataUrl?`<div class="qr-cell"><div class="qr-box"><img src="${e.contentQrDataUrl}" alt="QR" /><div class="cap">الأصناف والبيانات</div></div></div>`:``}
${e.qrDataUrl?`<div class="qr-cell"><div class="qr-box"><img src="${e.qrDataUrl}" alt="ZATCA QR" /><div class="cap">فواتير ZATCA</div></div></div>`:``}
</div>
<div class="main">
<table class="items"><thead><tr><th style="text-align:center;width:36px">م</th><th>الصنف</th><th style="text-align:center">الكمية</th><th style="text-align:center">السعر</th><th style="text-align:center">الإجمالي</th></tr></thead>
<tbody>${d||`<tr><td colspan="5" style="text-align:center;color:#94a3b8;background:#fff">لا توجد أصناف</td></tr>`}</tbody></table>
<div class="bottom">
...
${f.length?`<div class="custom-box">${f.map(([e,t])=>`<span>${_(e)}: ${_(String(t))}</span>`).join(``)}</div>`:``}
${p.length?`<div class="custom-box">${p.map(e=>`<span>${_(e.key)}: ${_(e.value)}</span>`).join(``)}</div>`:``}
...
${m||v?`<div class="pay-split"><div>كاش: <b>${a(m,s)}</b></div><div>شبكة: <b>${a(v,s)}</b></div></div>`:``}
</div>
</div>
</div>
