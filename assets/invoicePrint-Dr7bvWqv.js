...
<div class="name">${l(i.company_name||(a?`فاتورة بيع`:`فاتورة شراء`))}${k?`<div dir="ltr" style="font-size:12px;font-weight:400;color:rgba(255,255,255,.75);text-align:right">${l(k)}</div>`:``}</div>
<div class="sub">${[T,i.address,i.phone?`هاتف: ${i.phone}`:``,i.vat_no?`سجل/ضريبي: ${i.vat_no}`:``].filter(Boolean).join(` • `)||t(e.date)}</div>
</div>
</div>
<div class="doc-badge"><span class="t">${f}</span><div dir="ltr" style="font-size:10px;color:rgba(255,255,255,.7);font-weight:700">${a?`SALES INVOICE`:`PURCHASE INVOICE`}</div><div class="n">رقم: ${l(e.number||`—`)}</div></div>
</div>
${e.header_text||i.invoice_header_text?`<div class="note-box">${l(e.header_text||i.invoice_header_text||``)}</div>`:``}
<table class="doc">
<tr><td colspan="${A}" class="sec-title">${f} رقم ${l(e.number||`—`)} <span dir="ltr" style="font-size:10px;font-weight:400;opacity:.8">${a?`SALES`:`PURCHASE`} • ITEMS ${(e.items||[]).length}</span></td></tr>
<tr><td class="lbl">رقم الفاتورة</td><td class="val">${l(e.number||`—`)}</td><td class="lbl">التاريخ${e.due_date?` / الاستحقاق`:``}</td><td class="val" colspan="${j}">${t(e.date)}${e.due_date?` / ${t(e.due_date)}`:``}</td></tr>
<tr><td class="lbl">${a?`العميل`:`المورد`}</td><td class="val">${l(e.party_name||`عميل نقدي`)}${O?`<div dir="ltr" style="font-size:10px;color:#64748b;font-weight:400;text-align:right">${l(O)}</div>`:``}</td><td class="lbl">الفرع</td><td class="val" colspan="${j}">${l(e.branch_name||`—`)}</td></tr>
<tr><td class="lbl">الدفع</td><td class="val">${l(s[e.payment_method||``]||e.payment_method||`—`)}${S||C?` (كاش: ${n(S,p)} • شبكة: ${n(C,p)})`:``}</td><td class="lbl">الحالة</td><td class="val" colspan="${j}"><span class="status-pill">${c[e.status||``]||e.status||`—`}</span></td></tr>
<tr><td colspan="${A}" class="sec-title">الأصناف <span dir="ltr" style="font-size:10px;font-weight:400;opacity:.8">ITEMS (${(e.items||[]).length})</span></td></tr>
...
<tbody>${y||`<tr><td colspan="${A}" style="text-align:center;color:#94a3b8">لا توجد أصناف</td></tr>`}</tbody>
<tr class="grand"><td colspan="${A-2}">الإجمالي</td><td colspan="2">${n(m,p)}</td></tr>
<tr class="paidrow"><td colspan="${A-2}">المدفوع</td><td colspan="2">${n(h,p)}</td></tr>
<tr class="due"><td colspan="${A-2}">المتبقي</td><td colspan="2">${n(g,p)}</td></tr>
<tr><td class="lbl">المبلغ كتابة</td><td class="val" colspan="${M}">${l(u(m,p))}</td></tr>
${e.notes?`<tr><td class="lbl">ملاحظات</td><td class="val" colspan="${M}">${l(e.notes)}</td></tr>`:``}
${e.terms_text||i.invoice_terms_text?`<tr><td class="lbl">الشروط</td><td class="val" colspan="${M}">${l(e.terms_text||i.invoice_terms_text||``)}</td></tr>`:``}
${b.length?`<tr><td class="lbl">إضافي</td><td class="val" colspan="${M}">${b.map(({label:e,value:t})=>`${l(e)}: ${l(t)}`).join(` • `)}</td></tr>`:``}
${x.length?`<tr><td class="lbl">إضافي</td><td class="val" colspan="${M}">${x.map(e=>`${l(e.key)}: ${l(e.value)}`).join(` • `)}</td></tr>`:``}
${i.invoice_show_bank_details?`<tr><td class="lbl">البنك</td><td class="val" colspan="${M}">الحساب البنكي: ${l(i.phone||`—`)}</td></tr>`:``}
</table>
<div class="qr-row">
