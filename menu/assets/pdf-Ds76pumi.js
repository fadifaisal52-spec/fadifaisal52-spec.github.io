...
${r.map((e,t)=>`<tr><td class="c">${t+1}</td><td class="n">${e.name}${e.name_en?`<div dir="ltr" style="font-size:9px;font-weight:400;text-align:right">${e.name_en}</div>`:``}${e.size&&e.size!==`عادي`?` (${e.size})`:``}${e.notes?`<div style="font-size:9px;font-weight:400">📝 ${e.notes}</div>`:``}<div style="font-size:9px;font-weight:400">${Number(e.price).toFixed(2)}</div></td><td class="c">${e.qty}</td><td class="l">${(e.qty*e.price).toFixed(2)}</td></tr>`).join(``)}
</table>
<table class="tots">
